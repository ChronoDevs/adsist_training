...
|DB| Database→adsist_training_db<br>default/secret|

## file organization
```
/Works
├── laradock -- (docker files)
└── adsist_training -- (Work folder laravel is also placed here)
```
...
```diff
- APP_CODE_PATH_HOST=../
+ APP_CODE_PATH_HOST=../adsist_training
...
```SQL
CREATE DATABASE IF NOT EXISTS `adsist_training_testing` COLLATE 'utf8_general_ci';

GRANT ALL ON `adsist_training_testing`.* TO 'default'@'%';
FLUSH PRIVILEGES;
```
